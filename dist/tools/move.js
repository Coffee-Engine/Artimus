artimus.tools.move = class extends artimus.tool {
    ...
    selected(gl, previewGL, toolProperties) {
        ...
        if (this.workspace.hasSelection) this.imageData = gl.getImageData(selectionMinX, selectionMinY, selectionMaxX, selectionMaxY);
        else {
        ...
        createImageBitmap(this.imageData).then(bitmap => {
            this.bitmap = bitmap;
            this.ready = true;
            this.x = selectionMinX,
            this.y = selectionMinY,
    ...
    mouseDown(gl, x, y, toolProperties) {
        console.log(this.workspace.hasSelection);
        if (this.ready) {
            toolProperties.dragging = true;
        }
    }
