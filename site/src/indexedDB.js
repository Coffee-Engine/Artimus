...
(function () {
    const coffeeDBRequest = window.indexedDB.open(editor.dbName, editor.dbVersion);
    ...
    coffeeDBRequest.onupgradeneeded = (event) => {
        console.log(`Upgraded DB version to ${editor.dbVersion}`);
    ...
    coffeeDBRequest.onsuccess = (event) => {
        console.info("Connected to storage database");
        editor.indexedDB.available = true;
        editor.indexedDB.db = event.target.result;
